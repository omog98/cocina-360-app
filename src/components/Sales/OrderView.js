...
const OrderView = ({ table, activeOrder, onClose }) => {
  ...
  const updateQuantity = (productId, delta) => {
    setOrderItems(orderItems.map(item => {
      if (item.product_id === productId && item.isNew) {
  ...
  const removeItem = (productId) => {
    setOrderItems(orderItems.filter(item => !(item.product_id === productId && item.isNew)));
  };
  ...
  const handleSendToKitchen = async () => {
    try {
      ...
      if (idToDiscount) {
        try { await recipeService.discountInventory(idToDiscount); } catch (err) { console.error(err); }
      }
      ...
      setTimeout(async () => {
        try {
          ...
          const logoHTML = config?.logo_url 
            ? `<div style="text-align:center"><img src="${config.logo_url}" style="width:${logoWidth}px;margin-bottom:15px" /></div>` 
            : '';
          ...
          const ticketHTML = `
            ...
            <head>
                ...
                <style>
                    ...
                    * { font-weight: bold !important; }
                    body { font-family: 'Arial Black', 'Arial', sans-serif; font-size: ${fs * 1.5}px; width: 300px; margin: 0 auto; padding: 20px; color: #000; line-height: 1.4; }
            ...
            <body>
                ${logoHTML}
                <div class="center">
                    <div class="header-name">${config?.business_name || 'COCINA 360°'}</div>
                ...
                <div class="line"></div>
                <table>
                    <tr style="border-bottom:2px solid #000"><th>Cant</th><th>Producto</th><th class="right">Total</th></tr>
                    ${orderItems.map(item => `
                        <tr><td>${item.quantity}</td><td>${item.product_name}</td><td class="right">$${(item.price * item.quantity).toFixed(2)}</td></tr>
                    `).join('')}
                </table>
                <div class="line"></div>
    ...
    <div className="order-layout">
      ...
      <div className="order-panel">
        ...
        <div className="form-group" style={{ padding: '0 15px' }}>
          <input type="text" className="input" placeholder="Nombre del cliente..." value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
        </div>
          ...
          {orderItems.length === 0 ? (
            ...
            <>
              {orderItems.filter(i => !i.isNew).map((item, index) => (
                <div key={'sent-' + index} className="order-item" style={{ opacity: 0.8, borderLeft: item.isPromo ? '3px solid #e74c3c' : '3px solid #27ae60' }}>
                  <div className="item-info">
                    <span className="item-name">{item.product_name} {item.isPromo ? '🎁' : '✓'}</span>
              ...
              {orderItems.filter(i => i.isNew).map((item, index) => (
                <div key={'new-' + index} className="order-item" style={{ borderLeft: item.isPromo ? '3px solid #e74c3c' : '3px solid #FF6B35' }}>
                  <div className="item-info">
                    <span className="item-name">{item.product_name} {item.isPromo ? '🎁' : '🆕'}</span>
                    <span className="item-price">${item.price?.toFixed(2)} c/u</span>
                  </div>
        ...
        <div className="order-footer">
          ...
          {hasNewItems && (
            <button className="btn btn-warning btn-lg btn-full" onClick={handleSendToKitchen} style={{ marginBottom: 8 }}>
              🧾 Enviar a Cocina ({newItems.length})
            </button>
          )}
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="btn btn-danger btn-lg" style={{ flex: 1 }} onClick={handleCancelOrder}>❌ Cancelar</button>
            <button className="btn btn-success btn-lg" style={{ flex: 1 }} onClick={() => setShowPayment(true)} disabled={orderItems.length === 0}>
