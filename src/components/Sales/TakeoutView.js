...
const TakeoutView = ({ delivery = false }) => {
  ...
  const generateFolio = async () => {
    try { const data = await salesService.getNextFolio(); setFolio(String(data || 1)); } catch (error) { setFolio('1'); }
  };

  const addToOrder = (product) => {
    if (sentToKitchen) return;
    const existing = orderItems.find(item => item.product_id === product.id);
    // Usar delivery_price si es delivery y tiene precio especial
    const finalPrice = delivery && product.delivery_price > 0 ? product.delivery_price : product.price;
    if (existing) {
      ...
      setOrderItems([...orderItems, {
        product_id: product.id, product_name: product.name, price: finalPrice,
        quantity: 1, notes: '', preparation_time: product.preparation_time || 5, isPromo: false
      }]);
    }
    updateTime();
  };
  ...
  const updateTime = () => {
    const totalTime = orderItems.reduce((sum, item) => sum + ((item.preparation_time || 5) * item.quantity), 0);
  ...
  const handleSendToKitchen = async () => {
    try {
      ...
      const order = await salesService.createOrder({
        table_id: null, type: delivery ? 'delivery' : 'takeout',
        customer_name: customerName, customer_phone: customerPhone,
        folio: finalFolio, platform: delivery ? platform : null,
        estimated_time: estimatedTime, subtotal: finalTotal, tax: 0, total: finalTotal,
      ...
      for (const item of orderItems) {
        await salesService.addOrderItem({
          ...
          status: 'pending', sent_to_kitchen: true, preparation_time: item.preparation_time || 5
        });
      }
      setSentToKitchen(true);
    ...
    } catch (error) { console.error('Error:', error); showToast('Error al enviar a cocina', 'error'); }
  };
  ...
  const handlePayment = async (payments, tip) => {
    try {
      ...
      showToast('✅ Pedido cobrado - $' + finalTotal.toFixed(2)); clearOrder();
    } catch (error) { showToast('Error al cobrar', 'error'); }
  };

  const clearOrder = () => {
    setOrderItems([]); setCustomerName(''); setCustomerPhone('');
    setSentToKitchen(false); setOrderId(null); setEstimatedTime(0);
    setShowPayment(false); setActivePromo(null); setPlatform('uber');
    if (!delivery) generateFolio();
    ...
    <div className="view">
      <div className="view-header">
        <div>
          ...
          <p className="view-subtitle">{delivery ? getPlatformLabel(platform) + ' | Folio manual' : 'Folio #' + folio}</p>
        </div>
        <div>{estimatedTime > 0 && <span className="badge badge-warning" style={{ fontSize: 14, padding: '8px 15px' }}>⏱️ {estimatedTime} min</span>}</div>
      </div>
      ...
      <div className="order-layout">
        <div className="order-menu">
          {promos.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <button onClick={() => setShowPromos(!showPromos)} style={{ background: showPromos ? '#FF6B35' : 'var(--medium)', border: 'none', color: 'white', padding: '8px 15px', borderRadius: 8, cursor: 'pointer', fontWeight: 'bold', fontSize: 13 }}>
                🏷️ Promociones ({promos.length})
              </button>
              {showPromos && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', padding: '8px 0' }}>
                  {promos.map(promo => (
                    <button key={promo.id} onClick={() => handleApplyPromo(promo)} style={{ background: '#e74c3c', border: 'none', color: 'white', padding: '6px 12px', borderRadius: 15, cursor: 'pointer', fontSize: 12, fontWeight: 'bold' }}>
                      🏷️ {promo.name}
                    </button>
                  ))}
                </div>
            ...
            {filteredProducts.map(product => (
              <div key={product.id} className={`product-card ${sentToKitchen ? 'disabled' : ''}`} onClick={() => addToOrder(product)}>
                ...
                <div className="product-price">
                  ${delivery && product.delivery_price > 0 ? product.delivery_price?.toFixed(2) : product.price?.toFixed(2)}
                </div>
        ...
        <div className="order-panel">
          ...
          <div style={{ padding: '0 15px' }}>
            ...
            {delivery && (
              <div className="form-group"><label>Folio de App *</label><input type="text" className="input" placeholder="# de orden de la app..." value={manualFolio} onChange={(e) => setManualFolio(e.target.value)} disabled={sentToKitchen} /></div>
            )}
          </div>
          ...
          <div className="order-footer">
            <div className="totals">
              {estimatedTime > 0 && (<div className="total-row"><span>⏱️ Tiempo estimado:</span><span style={{ color: '#f39c12', fontWeight: 'bold' }}>{estimatedTime} min</span></div>)}
              {activePromo && (<div className="total-row" style={{ color: '#e74c3c' }}><span>🏷️ {activePromo.name}:</span><span>-${(total - finalTotal).toFixed(2)}</span></div>)}
              <div className="total-row grand-total"><span>TOTAL:</span><span>${finalTotal.toFixed(2)}</span></div>
            </div>
            {!sentToKitchen ? (
              <button className="btn btn-warning btn-lg btn-full" onClick={handleSendToKitchen} disabled={orderItems.length === 0}>🧾 Enviar a Cocina ({orderItems.length} productos)</button>
            ) : (
